console.log('=== CARRITO.JS INICIANDO - VERSIÓN CON LOGS ===');
...
function renderCarrito() {
    ...
    // Buscar diferentes elementos donde puede estar el carrito
    let tbody = document.getElementById("carrito-body");
    if (!tbody) {
        tbody = document.getElementById("productos-carrito");
    }
    if (!tbody) {
        tbody = document.querySelector("#carritoModal .modal-body tbody");
    }
    if (!tbody) {
    ...
    if (!tbody) {
        console.warn('No se encontró ningún elemento para renderizar el carrito');
    ...
    console.log('Elemento encontrado para renderizar:', tbody);
    ...
    // Buscar diferentes elementos donde puede estar el total
    let totalElement = document.getElementById("total");
    if (!totalElement) {
        totalElement = document.querySelector("#carritoModal #total");
    }
    if (!totalElement) {
        totalElement = document.querySelector(".modal-footer #total");
    }
    if (!totalElement) {
    ...
    if (totalElement) {
        ...
        console.log('Total actualizado en el DOM:', totalElement);
    } else {
        console.warn('No se encontró ningún elemento para mostrar el total');
        console.log('Elementos disponibles:', {
            total: document.getElementById("total"),
            carritoModal: document.getElementById("carritoModal"),
            modalFooters: document.querySelectorAll('.modal-footer')
        });
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('Documento cargado, iniciando carrito.js');
    ...
    function cerrarModalCarrito() {
        ...
        // También cerrar cualquier modal de Bootstrap que esté abierto
        ...
        // Limpiar cualquier overlay que pueda estar bloqueando
        document.body.style.overflow = '';
        document.body.classList.remove('modal-open');
        
        // Remover cualquier backdrop que pueda estar causando problemas
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') {
            console.log('Tecla Escape presionada, cerrando modales');
            cerrarModalCarrito();
            limpiarEstadoModalesCompleto();
        }
    });
    
    // Event listener global para clicks fuera de modales
    document.addEventListener('click', (e) => {
        // Si se hace click en un backdrop o elemento que debería cerrar modales
        if (e.target.classList.contains('modal-backdrop') || 
            e.target.classList.contains('modal') ||
            e.target.hasAttribute('data-bs-dismiss')) {
            console.log('Click detectado que debería cerrar modal');
            setTimeout(() => {
                limpiarEstadoModalesCompleto();
            }, 100);
        }
    ...
    if (carritoModal) {
        carritoModal.addEventListener('show.bs.modal', function () {
            ...
            // Detectar cuando se cierra el modal de Bootstrap
        carritoModal.addEventListener('hidden.bs.modal', function () {
            console.log('Modal de Bootstrap carritoModal cerrado');
            limpiarEstadoModalesCompleto();
        });
        
        // También agregar event listener para cuando se oculta
        carritoModal.addEventListener('hide.bs.modal', function () {
            console.log('Modal de Bootstrap carritoModal ocultándose');
            limpiarEstadoModalesCompleto();
        });
    }

    // Detectar cuando se cierra el modal de pago
    ...
    if (modalPago) {
        ...
        modalPago.addEventListener('hide.bs.modal', function () {
            console.log('Modal de pago ocultándose');
            limpiarEstadoModalesCompleto();
        });
    }
...
// Función global para limpiar el estado de modales
window.limpiarEstadoModales = function() {
    ...
    // Cerrar cualquier modal de Bootstrap que esté abierto
...
// Función más robusta para limpiar estado de modales
function limpiarEstadoModalesCompleto() {
    console.log('=== LIMPIANDO ESTADO DE MODALES COMPLETO ===');
    ...
    // Remover cualquier overlay personalizado
    const overlays = document.querySelectorAll('[id*="carrito-container"]');
    overlays.forEach(overlay => {
        if (overlay.style.display !== 'none') {
            console.log('Ocultando overlay:', overlay);
    ...
    setTimeout(() => {
        console.log('Limpieza adicional después de delay');
        document.body.style.overflow = '';
        document.body.classList.remove('modal-open');
        const backdropsDelayed = document.querySelectorAll('.modal-backdrop');
        backdropsDelayed.forEach(backdrop => backdrop.remove());
    }, 100);
}

// Intervalo para limpiar automáticamente cada 2 segundos si hay problemas
setInterval(() => {
    const backdrops = document.querySelectorAll('.modal-backdrop');
    const modalOpen = document.body.classList.contains('modal-open');
    
    if (backdrops.length > 0 || modalOpen) {
        console.log('Detectado estado problemático de modal, limpiando automáticamente');
        limpiarEstadoModalesCompleto();
    }
}, 2000);

// Verificar que la función esté disponible globalmente
console.log('=== FINAL DEL ARCHIVO CARRITO.JS ===');
...
console.log('Función limpiarEstadoModales disponible:', typeof window.limpiarEstadoModales !== 'undefined');
console.log('=== ARCHIVO CARRITO.JS COMPLETAMENTE CARGADO ===');
